...
def run_verification():
    ...
    for test_case in TEST_IDEAS:
        ...
        try:
            ...
            # -------------------------------------------------------------
            # TEST 3: ZERO MARKET SIZE SOURCES HONESTY CHECK
            # -------------------------------------------------------------
            print(f"\n  --> Testing ZERO MARKET SIZE SOURCES HONESTY for {name}...")
            no_market_sources = [s for s in sources if s.get("category") != "Market Size & Trends"]
            zero_m_res = market_agent.analyze(
                idea=idea_text,
                structured_idea=struct,
                sources=no_market_sources,
            )
            assert zero_m_res.market_size == [], f"Expected empty market_size, got {zero_m_res.market_size}"
            assert zero_m_res.attractiveness is None, f"Expected None attractiveness, got {zero_m_res.attractiveness}"
            assert zero_m_res.confidence is None, f"Expected None confidence, got {zero_m_res.confidence}"
            print(f"    PASS: Zero market size sources produced market_size: [], attractiveness: None, confidence: None.")
